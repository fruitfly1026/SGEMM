...
Our SGEMM achieves considerable performance
...
Figure~\ref{fig:rectangle} reports the performance of cuBLAS SGEMM and our optimized SGEMM.
...
The overall trend in Figure~\ref{fig:rectangle} is that SGEMM performance increases with matrix sizes. 
...
The number of threading blocks ranges in $2 \times 2, 8 \times 8, \dots, 64 \times 64$ from left to right in Figure~\ref{fig:rectangle}.
...
\begin{figure*}
    \begin{subfigure}[htbp]{0.3\textwidth}
        \includegraphics[width=2.5in]{tn_192}
        \subcaption{Matrix shape [W, W, W]}
        \label{fig:control_throughput}
    \end{subfigure}
    \begin{subfigure}[htbp]{0.3\textwidth}
        \includegraphics[width=2.5in]{1k2k4k}
...
Since $4\times 4$ leads to low data reuse in register files, we only show $8\times8$ and $12\times12$ cases in Figure~\ref{fig:rectangle}.
