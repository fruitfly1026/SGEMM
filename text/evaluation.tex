...
To hide shared memory latency, double-buffered software pipelining is used. Thus, extra $rx$ and $ry$ registers
...
shared
memory, global memory, and computation power. In the register blocking loop, each thread block computes $bm*bn*bk$ and reads $bm*bk+bn*bk$ words. The upper bound of global memory bandwidth can be modeled as:
\begin{equation}
...
For shared memory, inside each loop, $(rx*bk + ry * bk)*tx*ty$ words will be read from shared memory, in which $tx$ and
$ty$ are thread blocking dimensions, $rx$ and $ry$ are register blocking sizes. The computation is $bm*bn*bk$. Based on the ratio of computation to shared memory access,
\begin{equation}
