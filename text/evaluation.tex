...
When matrix size is xx,\jled{matrix size} the optimized SGEMM achieves $3104$ GFlop/s with $88\%$ efficiency, while cuBLAS gets $2509$ GFlop/s with $71\%$ efficiency. 
 %is $3104/3520=88\%$. 
...
On the other hand, a larger matrix has a better load balance on GPU by increasing the workload of the threading CUDA cores \jled{change to ``number of threading blocks''}. 
...
Thus, $12*12+4*12=192$ registers are used by {\tt LDx} and {\tt FFMA} instructions, and totally $236$ registers with other address indices\jled{what other?}.
...
Only one thread block per SM is active\jled{why?}, thus the thread occupancy is $256/1024=12.5\%$.
...
In this case, {\tt FFMA}s have $368/(144*4)=63.89\%$ \jled{2-way bank conflicts?} and $64/(144*4)=11.11\%$ 3-way bank conflicts. 
Besides, the baseline cannot apply dual issue optimization either.

{\it +Reg:}~~The register allocation pattern described in section~\ref{sec:assembler}\jled{refer to the subsection} is applied to eliminate register bank conflict. 
...
Single issue is controlled by setting control code to $0x00$ \jled{different from section~\ref{sec:benchmark}. The left sentence is useless.}. 
...
Compared to the baseline implementation, we gain $2.6\times$\jled{on average or maximum?} speedup by applying all the optimization.
...
\includegraphics[scale=0.35]{roofline}
    \caption{Roofline model using log-log scale.}
\label{fig:roofline}
\end{center}
\end{figure}
