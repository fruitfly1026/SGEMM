...
In this section, we compare the optimized SGEMM performance in Gflop/s with NVIDIA CUBLAS. The $17\sim 25\%$ 
improvement confirms that the microarchitectural optimization makes sense when tuning SGEMM on GPUs. We further present 
a quantitative analysis of the effect of each individual optimization strategy and an estimation of the upper bound 
performance.

The experiments are conducted on NVIDIA K20m GPU, and the hardware configuration is summarized in 
Table~\ref{table:k20}. The compared CUBLAS is from CUDA $7.0$ version. In our experiments the sizes of matrix vary in 
...
Our SGEMM achieves $3104$ Gflop/s which efficiency is $3104/3520=88\%$. With the same matrix size, CUBLAS has $2509$ 
Gflop/s which efficiency is $71\%$. The comparison shows that our implementation achives $17\%$ performance improvement 
over CUBLAS.

As the figure shows, the overall trend is that performance increases with size of matrices. On one hand, the larger 
