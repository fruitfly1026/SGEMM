...
The experiments are conducted on NVIDIA Tesla K20m GPU, xx (refer to \cite{xx}\jli{add} for detailed specification). 
\jli{Though delete the hardware configuration figure, still need to state main parameters in words!}
We compare with cuBLAS from CUDA $7.0$. In our experiments, we choose square matrices \jli{not only square matrices now} for $A$, $B$
...
SGEMM performance increases with matrix sizes in Figure~\ref{fig:sgemm_tn} . 
...
Figure~\ref{fig:1k2k4k}\jli{don't know why not showing number} shows SGEMM performance of four different matrix shapes, [w, 2w, 4w], [w, 4w, w], [4w, w, 4w], and [4w, 2w, w], where w values are shown on x-axis.
...
\begin{figure*}[!t]
\begin{center}
\includegraphics[width=\textwidth, trim={0 6.5in 0 0}, clip]{mp4}
...
The volume of data moving from shared memory to global memory is $r_x\times b_k+ r_y\times b_k$\jli{This is not shared to global memory, but shared to register in table~\ref{tab:dm}}, and the floating-point computation volume is $r_x\times r_y\times b_k$. 
...
Since $4\times 4$ leads to low data reuse in register files, we only show $8\times8$ and $12\times12$ cases in Figure~\ref{fig:sgemm_tn} and \ref{fig:1k2k4k}.
