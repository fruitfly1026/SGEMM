...
\begin{figure}[htbp]
\begin{center}
\includegraphics[scale=0.5]{sgemm_tn}
...
ry\times bk} {rx\times bk+ ry\times bk} = \frac{1}{\frac{1}{rx} + \frac{1}{ry}}.
    \label{equ:reg_block}
\end{equation}
Register blocking size is limited by number of registers per thread. 
Each thread needs $rx\times ry$, $rx$ and $ry$ registers to store the result of a sub-matrix of $C$,  a block-column of
$A$,  a block-row of $B$ in the same loop respectively.
In order to hide shared memory latency, double-buffered software pipelining is used. Thus, extra $rx$ and $ry$ registers
...
and the optimal solution lies at $rx=ry$ with restriction~\ref{equ:reg_restrict}. 
...
Performance of $12\times12$ is bettern than $8\times8$ because $12\times12$ blocking has higher $sAI$ and instruction
level parallelism. With respect to 
instruction scheduling optimization like Table~\ref{tab:position}, it has more slots to insert {\tt non-FFMA}
instructions, and latency can be hide easier.
...
By Euation~\ref{equ:global}, the flops:bytes ratio of $192\times192$ and $128\times128$ shared 
...
With our designed {\tt FFMA} dual issue pattern, every $6$ {\tt FFMA}s needs $4$ clock cycles in the pipeline.
It needs $4\times144\times4/6=384$ clocks for each thread, two {\tt LDG.128}s are needed.
We observe each {\tt LDG} has $10$ clocks penalty, the total {\tt LDG} will cause $2\times10/384 = 5.2\%$ loss. Other penalty 
