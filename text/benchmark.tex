...
\jled{Introduce a little microbenchmark}
...
For the eight control bits, we identify their meanings by statically examining CUBLAS disassembly codes of CUBLAS and dy
...
$4$th bit means global memory dependency barrier. Figure~\ref{fig:control_code}(\subref{fig:control_throughput}) and Fig
ure~\ref{fig:control_code}(\subref{fig:control_latency}) shows that {\tt FFMA} throughput and latency varies with all co
...
It's very intricate to tune instruction execution to improve instruction throughput. The previous work~\cite{lai} report
...
ig:control_code}(\subref{fig:control_pattern}). {\tt SDDS} behaves best, so we choose this pattern in our implementation
