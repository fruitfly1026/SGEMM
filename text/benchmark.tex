...
To explore NVIDIA Kepler architecture, we design microbenchmarks for each characteristic.
...
For timing accuracy, the same CUDA kernel code is tested for hundreds of times, and the average is used for analysis.
Timing ticks are recorded by {\tt S2R R0, SR\_CLOCKLO} instruction, in which
{\tt S2R} is to move the value in the special clock register to a general
...
Two metrics are benchmarked: latency and throughput. Latency is tested by a succession of dependented instructions~\cite{fog} by
...
\begin{table}[htbp]
    \caption{The efficiency of instruction throughput varies with different register indices. {\it Inst} :
...
\begin{itemize}
\item The destination register will not contribute to bank conflict, no matter which bank is assigned to.
...
The disassembly code from sample programs and cuBLAS library indicates that every $64$-bit control binary code controls $7$ instructions
...
We use crack the meaning of bits $0-3$ by testing the latency of {\tt FFMA} instruction.
...
Third, the first instruction of the core loop needs to be 8-instructions(seven
normal instructions plus one scheduling instruction) aligned. This restriction
...
In single mode, each warp scheduler can issue one float instruction to $32$ cores per clock, which
yields $4\times32/192=66.67\%$ float computation efficiency. 
In dual issue mode, two warp schedulers must use the shared cores alternately to avoid resource conflicts.
The jagged white and gray blocks show a proper phase shift between two warp's executing pace so that they could get
