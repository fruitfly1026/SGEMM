...
Last, each {\tt FFMA} requires three source registers, in dual issue, two {\tt FFMA} requires six source registers, but
...
Figure~\ref{fig:warp} illustrate the mapping of {\tt FFMA} instructions to cores in dual issue mode on one SM. There are $32\times 6=192$ cores on
...
We launch $26$ thread blocks each with $512$ threads, and specifies that each thread accesses four words in a stride of $4 \times blockDim.x \times gridDim.x$.
