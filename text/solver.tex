...
%We wrote simple PTX code generator, which generate PTX code with various modifiers specified.
%Then compile PTX file to cubin by {\tt ptxas}, and disassemble it by {\tt cuobjdump}. In this way, we generate real assembly
%instruction and their 64-bit encoding, which is the input of solver algorithm.
We disassmble libcublas.so to generate instruction and their corresponing 64-bit encodings, which are the input of
solver algorithm. There are $1537123$ different encodings of cublas 7.0 library in total.
An instruction consists of three fields: {\tt opcode}, {\tt operand} and {\tt modifier}. We express these information in a struct
varaible $instMap$.

An operand could be register, constant memory, global memory, shared memory, immediate values, or predicate register represented by different names.
...
\begin{algorithm}
  ...
  \begin{algorithmic}[1]
      ...
      \State \textbf {Onput:} Opcodes' binary positions for each input instruction
      \For {$i \gets 0, N$}
      ...
      \For {$j \gets 0, 63$}
	  \LineComment {if bit $j$ doesn't belong to any operand and it is 0 \jled{what about enc[j]!=0? No need to check if it is opcode?}}
