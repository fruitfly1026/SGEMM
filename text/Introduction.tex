...
``In convolutional neural network (CNN), the most expensive fully-connected or convolution layers are implemented using GEMM~\cite{chetlur2014cudnn}.
In the presence of statistical approximation and estimation errors, the computation of deep learning is not sensitive to high precision~\cite{Gupta}. In this work, we focus on SGEMM optimization.''}
...
microarchitecture by decoding instruction formats automatically, generating ISA-compatible executable \jled{change ``executable'' to ``binary''} codes, and benchmarking.
...
Although Kepler is not the latest generation GPU, the arithmetic dual-issue feature will not
outdate. 
During our exploring of the method to fully exploit the arithmetic
dual-issue feature of Kepler, we find that NVCC can not generate dual issue code efficiently even for computation
intensive SGEMM. Dual-issue feature would return in future GPU design when scalability of thread
level parallelism and frequency of GPU are too hard to improve.
