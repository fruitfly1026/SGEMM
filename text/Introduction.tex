...
Single floating-point precision General Matrix Multiply (SGEMM)\jli{further check}, as one 
...
demystify plenty of GPU microarchitecture features such as instruction issue, warp scheduling, register bank distribution, and control code\jli{+what?}, which helps understand and optimize the performance of a computational program. 
...
\item We design a microbenchmark suite to explore undocumented microarchitecture features of NVIDIA GPUs, such as control codes regulating {\tt FFMA} instruction dual issue and xx\jli{name another one, because it is a set of benchmarks}, which are necessary to understand and tune GPU programs.
