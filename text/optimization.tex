...
We apply 
a series of incremental optimizations to improve SGEMM efficiency\jled{to performance?} on Kepler architecture. The optimization strategies 
...
Figure~\ref{fig:assemblycode}, the instructions in lines 4-5 \jled{nothing in line 5. Check the line numbers.} and lines 7-8 are dual issued separately. 
...
cells denote {\tt FFMA} execution order. Dashed ellipses across two cells mean that two {\tt FFMA} instructions are dual issued in one clock cycle. \jled{Get rid of colors here.}}
\label{fig:order}
\end{center}
\end{figure}
...
$B$ bank $\begin{bmatrix} 2 & 3 \end{bmatrix}$ as shown in Figure\jled{which figure}, 2 choices left for $C$, 
...
According to our observations, we use {\tt LDG.E.128} to load data from global memory through texture cache and {\tt LDS.64} to load data from shared memory. 
We also have additional reasons to adopt them in SGEMM kernel. 
First, using {\tt LDG.E.128} reduces {\tt non-FFMA} \jled{use load instead of non-FFMA. Use non-FFMA seems complicated.} instructions. 
In Algorithm~\ref{gemm}'s inner loop, we need 3 {\tt LDG.E.128} instead of 12 32-bit load instructions to read $A$ and $B$.
Second, the shared memory transaction size is 256 bytes\jled{bytes or bits}, which forces each request being split into multiple transactions.
% it is another story for shared memory. 
If we use {\tt LDS.128}, we cannot infer the time of the second transaction by inspecting the inner loop, so it is difficult to eliminate potential bank conflicts between {\tt LDS.128}s and {\tt FFMA}s.
However, it's determinable for {\tt LDS.64}, since a transaction just happens at the position of it.\jled{don't understand this sentence.}
