...
An assembly microbenchmark suite correlates microarchitectural features with their performance factors to uncover instruction-level and memory hierarchy preferences.
% and uncovers float instruction dual issue, memory load width, register bank, and other 
We use SGEMM as a running example throughout this paper to show how to achieve bare-metal performance tuning. The performance boost is achieved by tuning {\tt FFMA} throughput to the hardware peak by activating dual issue, eliminating register bank conflicts, adding non-{\tt FFMA} instructions with little penalty, and choosing proper width of global/shared load instructions.
...
The toolchain is an attempt to automatically crack different GPU ISA encodings and build a complete assembler adaptively, to explore higher performance for applications on GPUs.
