...
In order to examine performance gain of different optimization strategies, we construct several implementation with incrementally applying the microarchitectural optimizations.

{\it Baseline:}~~Register for $C$ is allocated oderly from $0$ to $143$, then $A$ and $B$ matrices. In this case, {\tt
FFMA}s will have $368/(144*4)=63.89\%$ and $64/(144*4)=11.11\%$ 3-way bank conflicts.
...
We estimate the upper bound factors: {\tt LDS}, {\tt LDG} and {\tt FFMA}. The upper bound of global memory bandwidth can be modeled as:
\begin{displaymath}
\frac{bm*bn*bk}{bm*bk*rx + bn*bk*ry} > \frac{actual~flops}{bandwidth}
\end{displaymath}
According to parameters in our SGEMM implementation, $65$GB/s is the minimal requirement of global memory bandwidth to achieve peak $3520$ Gflops. Similarly, the minimum shared memory requirement is estimated as: (CHONGXIE!!!!!!)
\begin{displaymath}
M/unroll*reg_block*blockdim*gridx*gridy*2*bytes/time = 271GB/s
\end{displaymath}
For each SM, the minimum bandwidth requirement is $20$GB/s.
The microbenchmark indicates that the bandwidth we could achieve is higher than requirement, so bandwidth of shared width and global memory is not bottleneck.
