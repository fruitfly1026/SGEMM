...
{\it Baseline:} The baseline SGEMM implementation apply state of art SGEMM optimization like register blocking, global
memory double buffer , shared memory double buffer and unrolling, but will not optimize in assembly level optimization.
For example, baseline use default $32-bit$ {\tt LD} rather than $128-bit$ {\tt LDG} to load data from global memory.
Register allocation for $C$ is allocated oderly from $0$ to $143$, then $A$ and $B$ matrices. In this case, {\tt
...
{\it +Reg:}
Optimization with no register bank conflict is using register allocation pattern described in section~\ref{}. No
...
L1-cached, it may be L2-cached.

{\it +LDG128:} {\tt LD} is load instruction used in implenetation. When {\tt LDG} is used, a {\tt TEXDEPBAR} is needed
before using the data. Since change in instruction number, instruction scheduling is needed.

{\it +Dual:} Single issue is controled by setting control code to $0x00$. Dual issue is  using dual issue pattern
...
For Kepler device, the minimum bandwidth requirement is $1173$GB/s, and for each SM of Kepler, bandwidth requirement is
$1173/13=90$ GB/s.
The hardware can provide $200$GB/s global memory bandwidth and $2349$GB shared memory bandwidth, which is
higher than requirement, and hence neither bandwidth of shared width nor global memory will be bottleneck.

The loss to peak performance can be explained as the following reasons. As we have shown in Section, FFMA throughput can achieve $97.67\%$. The loss is about $2.33\%$, which may comes from overhead of warp scheduler in {\tt FFMA} dual issue mode. The double-buffering algorithm can amortize the latency of {\tt LDS}.
