...
\label{table:k20}
\end{table}
Our platform is K20m GPU, the configuration of K20 GPU is shown in Table~\ref{table:k20}.
Cuda driver and Cublas is $7.0$ version. We benchmark different size of matrix from $768\times768$ to $12288\times12288$.
...
efficiency is $2216/3520=63\%$. Our SGEMM TN implementation is 25\% higher than Cublas's TN.
We can see the performance increase with matrix size, it's due to load balance and epilog overhead of storing $C$
matrix.
Numbers of block ranges from $[768/192,768/192]=[4,4]$ to $[12288/192, 12288/192]$ $=[64,64]$. Kepler has $13$ SMs in
total, matrix of size $768x768$ will have severe load imbalance problem. Other matrices have more thread blocks, load balance
problem is serious.
\begin{figure}[htbp]
\begin{center}
...
\subsubsection{Profiling of Each Optimization Method}
...
\end{center}
\end{figure}
{\bf conf/noconf:} Register for $C$ is allocated oderly from $0$ to $143$, then $A$ and $B$ matrices. In this case, {\tt
FFMA}s will have $368/(144*4)=63.89\%$ and $64/(144*4)=11.11\%$ 3-way bank conflicts. Optimization with no register bank
conflict(noconf) is using register allocate pattern described in section~\ref{}. No instruction scheduling change
between conf and nonconf version.

\textbf{ld/ldg+N:} {\tt LD} is load instruction used in implenetation. When {\tt LDG} is used, a {\tt TEXDEPBAR} is needed
before using the data. Since change in instruction number, instruction scheduling is needed.
Kepler has an L1 data cache, but it is designed for local rather than global memory access.

{\bf single/dual:} Single issue is controled by setting control code to $0x00$. Dual issue is  using dual issue pattern
we described in section~\ref{}. For dual issue version, $NOP$ may be inserted for instruction alignment. No instruction
scheduling change betwen single/dual.
...
Figure~\ref{fig:block} demonstrates $8\times8$ blocking and $12\times12$ blocking. $12\times12$ is better than
$8\times8$ in that, it has high computation memory ratio and higher instruction level parallelism. In terms of
optimization methods, $12\times12$ blocking has more slots to insert other {\tt non-FFMA} instructions, leaving more
space to do instruction scheduling, the overhead of {\tt LDS} can be eliminated.
\begin{figure}[htbp]
\begin{center}
...
We anlysis the possible upper bound factor of SGEMM: {\tt LDS}, {\tt LDG} and {\tt FFMA} throughput.
In section~\ref{}, {\tt LDS} theoretical bandwidth is xxx, the maximize we could achive is $160GB/s$.
...
The bandwidth we could achiev is higher than requirement, so bandwidth of shared width and global memory is not bottleneck.

The best performance we could achive is $88\%$ efficiency.
As we have shown in Section~\ref{sec:kepleras}, FFMA throughput can achieve $97.67\%$.
The loss is about $2.33\%$. These loss may comes from overhead of warp scheduler in FFMA dual issue
mode. LDS has no penalty. With register double buffering, we can eliminate all the influence of LDS or STS.
With $12x12$ register blocking, unroll $4$ times, there will be $144*4=576$ FFMA instructions in the loop.
With our designed FFMA dual issue pattern, every $6$ FFMA needs $4$ clock cycles in the pipeline.
It needs $4*144*4/6=384$ clock cycles for each thread,  two $128$ bits LDG instruction are needed.
Each LDG has $10$ clock penalty, the total LDG penalty will be $2*10/384 = 5.2\%$.
Other penalty is about 13\%-5.2\%-2.33\%=5.5\%. It comes from synchronization and writing $C$ matrix in the block.
%
% The following two commands are all you need in the
% initial runs of your .tex file to
% produce the bibliography for the citations in your paper.
\section{Related Work}
GEMM is a basic low level function of BLAS, its performance depends on architecture heavily.
There are lots of excellent research work on it.
Volkov et.al.\cite{volkov}
presented a fast implementation on G80 architecture. A
blocking algorithm was used to exploit locality in shared
memory. They also presented detailed benchmarks of the
GPU memory system, kernel start-up costs, and arithmetic
throughput. However, we present a performance modeling
and a detailed analysis on the rationality of choosing an
optimal blocking factor, and an experience on instruction
scheduling as well.
Nath et.al.~\cite{magma} has developed a dense
linear algebra library MAGMA for heterogeneous architectures of multi-core+GPU. The library contains a DGEMM
routines optimized for Fermi. MAGMA adopts a similar
idea of double-buffering by prefetching data into extra registers instead of shared memory. MAGMA’s performance
is almost the same as CUBLAS3.2.
Wong et.al.~\cite{wong}  performed a comprehensive benchmarking work on GT200.
Tan et.al.~\cite{tan} implemented fast DGEMM by using assembly level optimization, such as
software pipeling, use of vector memory operations, register blocking, instruction order schedualing, double-buffering.
Lai~\cite{lai} present performance analysis and optimization work of SGEMM on fermi and
GTX680 GPU architecture.
Scott Gray~\cite{nervana_sgemm_wiki} present optimization of SGEMM and other kernels used in
convolutional neural network.
Mei~\cite{mei} benchark memory hierachy of fermi, Kepler and Maxwell GPU, which including cache,
shared memory etc.
\section{Conclusion}
